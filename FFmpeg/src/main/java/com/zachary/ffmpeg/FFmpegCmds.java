...
public class FFmpegCmds {
    ...
    public static String[] mixAudio(String inputPath, String mixFile, String outputPath) {
        ...
        String mixAudioCmd = "ffmpeg -i %s -i %s -filter_complex amix=inputs=2:duration=first -strict -2 -y %s";
    ...
    /**
     * delay one audio, delayed channels are filled with silence
     * @param inputPath
     * @param delaySeconds
     * @param outputPath
     * @return
     */
    public static String[] delayAudio(String inputPath, int delaySeconds, String outputPath) {
        String mixAudioCmd = "ffmpeg -i %s -filter_complex adelay=delays=%ds:all=1 -strict -2 -y %s";
        mixAudioCmd = String.format(Locale.CHINA, mixAudioCmd, inputPath, delaySeconds, outputPath);
        return mixAudioCmd.split(" ");
    }
